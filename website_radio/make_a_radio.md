...
- | 问题     | 断点A（V2） | 断点B（V5） | 断点C（V8） | 断点D（V9/V10） |
  | -------- | ----------- | ----------- | ----------- | --------------- |
  | 电流过小 | 调小R1阻值  | 调小R4阻值  | 调小R9阻值  | 调小R12阻值     |
  | 电流过大 | 调大R1阻值  | 调大R4阻值  | 调大R9阻值  | 调大R12阻值     |
...
- 电路板元器件识别：
  ...
  > 1.电阻（图中的电阻要倒过来看，因为最后一个环不可能是红色。具体要不要倒过来看以实物为准）
  >
  > 2.电阻，电阻的读数方法见P38色标法表格
  >
  > 3、4：电容，电容见P43
  >
  > 5：可调磁芯线圈（电感） 6：固定电感，电感见P48
  >
  > 7：硅二极管，整流，灰色处为正级 8：锗二极管，检波（特征：第二个字母为A），极性在侧面标注 二极管见P56
  >
  > 9、10：三极管，平面朝自己，脚朝下，从左到右依次是e、b、c，按照平面标注的型号和图纸的标注判断示意图。图中9号为9018H，NPN，在图纸中央位置；10号为9012G，PNP，在下方的`G(+)`字样上方。详见P61。
  >
  > 11、12：三极管，从上往下依次为e、c、b。详见P61。
  >
  > 13：1206片式电阻，1/16W，510Ω。详见P190
  >
  > 14：三极管，上c左b右e。
  >
  > 15：二极管
